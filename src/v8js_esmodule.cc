...
v8::MaybeLocal<v8::Module> v8js_module_resolve_callback(
	...
	if (getenv("V8JS_DEBUG_MODULES")) {
		php_printf("RESOLVE: %s\n", specifier_cstr);
	}
...
v8::MaybeLocal<v8::Module> v8js_load_module(
	...
	if (EG(exception)) {
		...
		// Get exception message - use the exception's message property directly
		std::string error_msg = "Module not found";
		zval *message_prop = zend_read_property(zend_ce_exception, EG(exception), ZEND_STRL("message"), 0, NULL);
		if (message_prop && Z_TYPE_P(message_prop) == IS_STRING) {
			error_msg = std::string(Z_STRVAL_P(message_prop), Z_STRLEN_P(message_prop));
		}
		
		// Clear the PHP exception so it doesn't bubble up
		zend_clear_exception();
		
		// Throw V8 exception with the message
		isolate->ThrowException(V8JS_SYM(error_msg.c_str()));
...
bool v8js_instantiate_module(
	...
	if (getenv("V8JS_DEBUG_MODULES")) {
		// Find module name
		for (auto &pair : c->esmodules_loaded) {
			if (pair.second.Get(isolate)->GetIdentityHash() == module->GetIdentityHash()) {
				php_printf("INSTANTIATE: %s (status=%d)\n", pair.first.c_str(), module->GetStatus());
				break;
			}
		}
	}
...
v8::MaybeLocal<v8::Promise> v8js_module_dynamic_import_callback(
	...
	// Check if evaluation returned a promise (top-level await)
	v8::Local<v8::Value> result_value = result.ToLocalChecked();
	
	if (result_value->IsPromise()) {
		v8::Local<v8::Promise> eval_promise = result_value.As<v8::Promise>();
		
		// Chain the promises: when evaluation completes, resolve with module namespace
		v8::Local<v8::Function> then_handler;
		v8::Local<v8::Function> catch_handler;
		
		// Store module identity hash to retrieve it in callback
		int module_hash = module->GetIdentityHash();
		v8::Local<v8::Integer> hash_data = v8::Integer::New(isolate, module_hash);
		
		// Create a fulfillment handler that returns the module namespace
		auto then_callback = [](const v8::FunctionCallbackInfo<v8::Value>& info) {
			v8::Isolate* isolate = info.GetIsolate();
			v8js_ctx *c = (v8js_ctx *) isolate->GetData(0);
			v8::Local<v8::Context> context = isolate->GetCurrentContext();
			
			// Get the module hash from the data
			int module_hash = info.Data().As<v8::Integer>()->Value();
			
			// Find the module by hash
			for (auto &pair : c->esmodules_loaded) {
				v8::Local<v8::Module> mod = v8::Local<v8::Module>::New(isolate, pair.second);
				if (mod->GetIdentityHash() == module_hash) {
			...
			// Module not found - shouldn't happen
			isolate->ThrowException(V8JS_SYM("Module not found in callback"));
		};
		
		// Create a rejection handler
		auto catch_callback = [](const v8::FunctionCallbackInfo<v8::Value>& info) {
			// Pass through the rejection reason
			if (info.Length() > 0) {
				info.GetIsolate()->ThrowException(info[0]);
			}
		};
		
		then_handler = v8::Function::New(context, then_callback, hash_data).ToLocalChecked();
		catch_handler = v8::Function::New(context, catch_callback).ToLocalChecked();
		
		// Chain the promises
		v8::Local<v8::Promise> chained = eval_promise->Then(context, then_handler, catch_handler).ToLocalChecked();
		
		// Resolve our promise with the chained promise's result
		resolver->Resolve(context, chained).Check();
		
	} else {
		// No top-level await, return the module namespace immediately
