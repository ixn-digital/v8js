...
static void v8js_array_access_length(v8::Local<v8::String> property, const v8::PropertyCallbackInfo<v8::Value>& info) /* {{{ */
{
	...
	// V8 13+ sandbox safety check
	if (!v8js_is_valid_wrapped_object(self)) {
		info.GetReturnValue().Set(V8JS_INT(0));
		return;
	}
...
V8JS_INTERCEPTED v8js_array_access_named_getter(v8::Local<v8::Name> property_name, const v8::PropertyCallbackInfo<v8::Value> &info) /* {{{ */
{
	...
	v8::Local<v8::Value> ret_value = v8js_named_property_callback(info.GetIsolate(), info.HolderV2(), property, V8JS_PROP_GETTER);

	if(ret_value.IsEmpty()) {
		v8::Local<v8::Array> arr = v8::Array::New(isolate);
		v8::Local<v8::Value> prototype = arr->GetPrototypeV2();
