...
Windows is currently _not_ officially supported. Mostly since I don't have the time to
...
V8Js includes two test suites:

### PHPT Tests

Standard PHP extension tests using the PHPT format:
...
V8Js includes integration with [Test262](https://github.com/tc39/test262), the official
ECMAScript conformance test suite. This helps verify that V8js correctly implements
JavaScript features according to the ECMAScript specification.

To run Test262 tests:

```bash
# Initialize the test262 submodule
...
See [tests/test262/README.md](tests/test262/README.md) for detailed documentation
on Test262 integration, configuration, and interpreting results.
...
PHP arrays are immediately exported value by value without live binding. This is if you change a value on JavaScript side or push further values onto the array, this change is _not_ reflected on PHP side.
...
The `executeString` function can be configured to always map JavaScript objects to PHP arrays by setting the `V8Js::FLAG_FORCE_ARRAY` flag. Then the standard array behaviour applies that values are not live-bound, i.e. if you change values of the resulting PHP array, the JavaScript object is _not_ affected.
...
from PHP, since they are added right _after_ the snapshot code is run.
...
_not_ re-thrown into JavaScript context but cause the JavaScript execution to
